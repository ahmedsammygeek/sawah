        ...
        <div id="table-default" class="table-responsive">
            <table class="table">
                ...
                <tbody class="table-tbody">
                    ...
                    <tr>
                        <td> {{ $i++ }} </td>
                        <td> {{ $offer->getTranslation('name' , 'ar') }} </td>
                        <td> {{ $offer->getTranslation('name' , 'en') }} </td>
                        <td> {{ $offer->created_at }} <span class="text-muted">{{ $offer->created_at->diffForHumans() }}</span> </td>
                        <td> {{ $offer->user?->name }} </td>
                        <td>
                            @switch($offer->is_active)
                            @case(1)
                        ...
                        <td class='row g-2 align-items-center' >
                            <div class='col-6 col-sm-4 col-md-2 col-xl-auto '>
                                <a href="{{ route('board.offers.show' , $offer) }}" class="btn btn-primary w-100 btn-icon" aria-label="Facebook">
                            ...
                            <div class='col-6 col-sm-4 col-md-2 col-xl-auto '>
                                <a href="{{ route('board.offers.edit' , $offer ) }}" class="btn btn-warning w-100 btn-icon" aria-label="Facebook">
                            ...
                            <div class='col-6 col-sm-4 col-md-2 col-xl-auto '>
                                <a class="btn btn-danger w-100 btn-icon" wire:click="$emit('confirmDeletion' , {{ $offer->id }} )" aria-label="Facebook">
