...
<div class="page-body">
	<div class="container-xl">
		<div class="row row-deck row-cards">
			<div class="col-md-12">
				<form class="card" method='POST' action='{{ route('board.goals.update', $goal ) }}' enctype="multipart/form-data" >
					@csrf
					@method('PATCH')
					<div class="card-header bg-primary">
						<h3 class="card-title text-white"> تعديل بيانات الهدف </h3>
					</div>
					<div class="card-body">
						...
						<div class="mb-3">
							...
							<div>
								<input type="text" class="form-control @error('title') is-invalid @enderror " name='title' value="{{ $goal->getTranslation('title' , 'ar') }}" >
								@error('title')
								<small class="form-hint text-danger"> {{ $message }} </small>
								@enderror
							</div>
						...
						<div class="mb-3">
							...
							<div>
								<textarea  class="form-control @error('content') is-invalid @enderror " name='content'  cols="30" rows="10"> {{ $goal->getTranslation('content' , 'ar') }} </textarea>
								@error('content')
								<small class="form-hint text-danger"> {{ $message }} </small>
								@enderror
							</div>
					...
					<div class="card-footer text-end">
						<div class='d-flex' >
							<a href="{{ route('board.goals.index') }}" class="btn btn-link"> تراجع </a>
