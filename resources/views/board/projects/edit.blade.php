			...
			<div class="col-md-12">
				<form class="card" method='POST' action='{{ route('board.projects.update' , $project ) }}' enctype="multipart/form-data" >
					...
					<div class="card-header bg-primary">
						<h3 class="card-title text-white"> تعديل  المشروع  </h3>
					</div>
					<div class="card-body">
						<div class="mb-3">
						...
						<div class="mb-3">
							...
							<div>
								<input type="text" class="form-control @error('name_ar') is-invalid @enderror " name='name_ar' value="{{ $project->getTranslation('name' , 'ar' ) }}" >
								@error('name_ar')
								<small class="form-hint text-danger"> {{ $message }} </small>
								@enderror
							</div>
						...
						<div class="mb-3">
							...
							<div>
								<input type="text" class="form-control @error('name_en') is-invalid @enderror " name='name_en' value="{{ $project->getTranslation('name' , 'en' ) }}" >
								@error('name_en')
								<small class="form-hint text-danger"> {{ $message }} </small>
								@enderror
							</div>
						...
						<div class="mb-3">
							...
							<div>
								<label class="form-check">
									<input class="form-check-input" name='active' type="checkbox" {{ $project->is_active == 1 ? 'checked' : '' }}>
									<span class="form-check-label"> السماح بالعرض </span>
								</label>
							</div>
						</div>
						<div class="mb-3">
							<label class="form-label"> صوره المشروع الحاليه </label>
							<div>
