...
<div class="page-body">
	<div class="container-xl">
		<div class="row row-deck row-cards">
			<div class="col-md-12">
				<form class="card" method='POST' action='{{ route('board.offers.update' , $offer ) }}' >
					@csrf
					@method('PATCH')
					<div class="card-header bg-primary">
						<h3 class="card-title text-white"> تعديل العرض </h3>
					</div>
					<div class="card-body">
						<div class="mb-3">
							<label class="form-label required"> العنوان بالعربيه </label>
							<div>
								<input type="text" class="form-control @error('name_ar') is-invalid @enderror " name='name_ar' value="{{ $offer->getTranslation('name' , 'ar') }}" >
								@error('name_ar')
						...
						<div class="mb-3">
							...
							<div>
								<input type="text" class="form-control @error('name_en') is-invalid @enderror " name='name_en' value="{{ $offer->getTranslation('name' , 'en') }}" >
								@error('name_en')
								<small class="form-hint text-danger"> {{ $message }} </small>
								@enderror
							</div>
						...
						<div class="mb-3">
							...
							<div>
								<input type="text" class="form-control @error('content_ar') is-invalid @enderror " name='content_ar' value="{{ $offer->getTranslation('content' , 'ar') }}" >
								@error('content_ar')
								<small class="form-hint text-danger"> {{ $message }} </small>
								@enderror
							</div>
						...
						<div class="mb-3">
							...
							<div>
								<input type="text" class="form-control @error('content_en') is-invalid @enderror " name='content_en' value="{{ $offer->getTranslation('content' , 'en') }}" >
								@error('content_en')
								<small class="form-hint text-danger"> {{ $message }} </small>
								@enderror
							</div>
						...
						<div class="mb-3">
							...
							<div>
								<label class="form-check">
									<input class="form-check-input" name='active' type="checkbox"  {{ $offer->is_active == 1 ? 'checked' : '' }} >
